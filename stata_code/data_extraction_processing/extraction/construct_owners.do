...
odbc load,  exec("select distinct(b.person_id), b.business_name, c.business_id, a.vp_num, a.ap_year, ves.hull_id
	from nefsc_garfo.permit_vps_owner c, nefsc_garfo.client_bus_own b, nefsc_garfo.permit_vps_fishery_ner a, nefsc_garfo.permit_vps_vessel ves
		where c.ap_num in (select max(ap_num) as ap_num from nefsc_garfo.permit_vps_vessel where ap_year between $firstyr and $lastyr group by vp_num, ap_year)
	 and c.business_id=b.business_id and a.ap_num=c.ap_num and a.ap_num=ves.ap_num;") $oracle_cxn;
...
save "${my_workdir}/ownership2010_present_${today_date_string}.dta", replace;

#delimit; 
clear;
odbc load,  exec("select pids.*, ves.hull_id from nefsc_garfo.permit_owner_history_pids pids, nefsc_garfo.permit_vps_vessel ves
     where  pids.ap_num in (select max(ap_num) as ap_num from nefsc_garfo.permit_owner_history_pids group by permit, ap_year) 
	 and pids.ap_num=ves.ap_num;") $oracle_cxn;
destring , replace;
drop if ap_year==.;
...
save "${my_workdir}/ownership_pre2010_${today_date_string}.dta", replace;



clear;

odbc load,  exec("select business_id, business_name, person_id, name_first, name_middle, name_last from nefsc_garfo.client_bus_own;") $oracle_cxn;
destring, replace;
